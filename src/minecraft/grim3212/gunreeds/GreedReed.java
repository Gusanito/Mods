...
public class GreedReed extends Item
{
    /** The ID of the block the reed will spawn when used from inventory bar. */
    private int spawnID;

    public GreedReed(int par1, Block par2Block)
    {
        super(par1);
        this.spawnID = GunpowderReed.greedBlockID;
        this.setCreativeTab(CreativeTabs.tabMaterials);
    }
    
    public void updateIcons(IconRegister par1IconRegister) {
		this.iconIndex = par1IconRegister.registerIcon("GunpowderReeds:" + "greedItem");
	}
    ...
    public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
    {
        ...
        else
        {
            if (par3World.canPlaceEntityOnSide(this.spawnID, par4, par5, par6, false, par7, (Entity)null, par1ItemStack))
            {
                ...
                if (par3World.setBlock(par4, par5, par6, this.spawnID, var13, GunpowderReed.greedBlock.blockID))
                {
